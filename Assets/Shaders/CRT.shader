...
Shader "PostEffects/CRT"
{
	Properties
	{
		...
		_ScanLineInterval("TailInterval", Float) = 500
		_ScanLineInterval("TailBrightness", Float) = 500
	}
		SubShader
		{
			// No culling or depth
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					// スキャンラインを描画
					float scanLineColor = sin(_Time.y * 10 + uv.y * _ScanLineInterval) / 2 + 0.5;
					col *= _ScanLineBrightness + clamp(scanLineColor + 0.8, 0, 1) * 0.5;
